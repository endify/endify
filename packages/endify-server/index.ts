...
import {HookService} from '../endify-tools/HookService/HookService'
...
async function appFactory() {
  const app = express()
  await hookService.call(ServerHookTypes.BeforeClientMiddleware, {app})
  app.use('/', await endifyVueMiddleware(configService.config.vue))
  await hookService.call(ServerHookTypes.AfterClientMiddleware, {app})
  return app
}
